...
import {Country as CustomCountry} from "@/actions/getUserCountry";
...
import changeUserCountry from "@/actions/change-user-country";
...
interface CountryDropdownProps {
    ...
    placeholder?: string
    slim?: boolean
    defaultCountry?: CountryInput

}

type CountryInput = Country | CustomCountry

function normalizeCountry(input: CountryInput | undefined): Country | undefined {
    ...
    return countries.all.find(
        (c) =>
            c.alpha2 === input.country_code ||
            c.name.toLowerCase() === input.country.toLowerCase()
    )
}

function toCustomCountry(c: Country): CustomCountry {
    return {
        country: c.name,
...
const CountryDropdownComponent = (
    ...
    useEffect(() => {
        if (defaultCountry) {
            const normalized = normalizeCountry(defaultCountry)
    ...
    }, [defaultCountry, defaultValue, options])
    ...
    const handleSelect = useCallback(
        async (country: Country) => {
            ...
            onChange?.(country)
            await changeUserCountry(toCustomCountry(country))
            setOpen(false)
        },
        [onChange],
    )
            ...
            <PopoverTrigger ref={ref} className={triggerClasses} disabled={disabled} {...props}>
                {selectedCountry ? (
                    <div className="flex items-center flex-grow w-0 gap-2 overflow-hidden">
                        ...
                        {slim === false && (
                            <span
                                className="overflow-hidden text-ellipsis whitespace-nowrap">{selectedCountry.name}</span>
                        )}
                    </div>
                ) : (
                                ...
                                .map((option, key: number) => (
                                    ...
                                    >
                                        <div className="flex flex-grow w-0 space-x-2 overflow-hidden">
                                            ...
                                            <span
                                                className="overflow-hidden text-ellipsis whitespace-nowrap">{option.name}</span>
                                        </div>
                                        <CheckIcon
                                            className={cn(
